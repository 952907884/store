...
$(function () {
    ...
    function Subclass(data) {
        ...
        for (var i=0; i<obj.length ; i++) {
            console.log(obj[i].subclassName);
            var disable=obj[i].disable==1?"停用":"使用";
            var lastchange=obj[i].lastchange==undefined?"0":obj[i].lastchange;
            $("#table-content").append(
                "<tr><td><input type='checkbox' /></td>" +
                "<td class='am-text-center'>"+obj[i].subclassid+"</td><td><i style='margin-left:30px'>├─  </i><a href='#'>"+obj[i].subclassName+"</a></td>" +
                "<td>"+disable+"</td>" +
    ...
    function addPropertyConent(data) {
        ...
        $("#form-nav").after(
            ...
            '<th class="table-id am-text-center">小类编号</th>'+
            '<th class="table-title">小类名称</th>'+
